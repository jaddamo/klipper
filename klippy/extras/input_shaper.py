...
class InputShaperParams:
    def __init__(self, axis, config):
        ...
        self.shaper_type = config.get(f'shaper_type_{axis}', shaper_type)
        if self.shaper_type not in self.shapers:
            raise config.error(f'Unsupported shaper type: {self.shaper_type}')
        self.damping_ratio = config.getfloat(
            f'damping_ratio_{axis}',
            shaper_defs.DEFAULT_DAMPING_RATIO,
            minval=0.0,
            maxval=1.0,
        )
        self.shaper_freq = config.getfloat(f'shaper_freq_{axis}', 0., minval=0.)
    def update(self, gcmd):
        axis = self.axis.upper()
        self.damping_ratio = gcmd.get_float(
            f'DAMPING_RATIO_{axis}', self.damping_ratio, minval=0.0, maxval=1.0
        )
        self.shaper_freq = gcmd.get_float(
            f'SHAPER_FREQ_{axis}', self.shaper_freq, minval=0.0
        )
        shaper_type = gcmd.get('SHAPER_TYPE', None)
        if shaper_type is None:
            shaper_type = gcmd.get(f'SHAPER_TYPE_{axis}', self.shaper_type)
        if shaper_type.lower() not in self.shapers:
            raise gcmd.error(f'Unsupported shaper type: {shaper_type}')
        self.shaper_type = shaper_type.lower()
    def get_shaper(self):
        A, T = (
            self.shapers[self.shaper_type](self.shaper_freq, self.damping_ratio)
            if self.shaper_freq
            else shaper_defs.get_none_shaper()
        )
        return len(A), A, T
    def get_status(self):
...
class InputShaper:
    ...
    def _update_input_shaping(self, error=None):
        ...
        if failed:
            ...
            raise error(
                f"Failed to configure shaper(s) {', '.join([s.get_name() for s in failed])} with given parameters"
            )
    def disable_shaping(self):
        for shaper in self.shapers:
